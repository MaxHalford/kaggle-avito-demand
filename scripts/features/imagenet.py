...
def export_features(imagenet):
    ...
    df['object_0_probability'].fillna(
        df['object_0_probability'].mean(), inplace=True)
    df['object_1_probability'].fillna(
        df['object_1_probability'].mean(), inplace=True)
    df['object_2_probability'].fillna(
        df['object_2_probability'].mean(), inplace=True)
    df['object_3_probability'].fillna(
        df['object_3_probability'].mean(), inplace=True)
    df['object_4_probability'].fillna(
        df['object_4_probability'].mean(), inplace=True)
...
def extract_probability(files, input_shape, preprocess, model):
    ...
    for f in tqdm(files):
        ...
        with archive.open(f) as file:
            ...
            if compteur == 100000:
                export_features(imagenet).to_csv(
                    'features/train/imagenet.csv', index=False)
                compteur = 0
            ...
            try:
                ...
                for i in range(len(P[0])):
                    ...
                    imagenet[name]['object_{}_probability'.format(
                        i)] = P[0][i][2]
